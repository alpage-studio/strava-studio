...
Studio.template({
  ...
  draw: function (s) {
    ...
    if (o.voile) {
      H.scrimRadial(1.20, 0.70, 0.50, 0.42, [
        [0, 'rgba(0,0,0,.42)'], [0.72, 'rgba(0,0,0,0)']
      ]);
    }
    ...
    H.text(f.name, cx, H.bl(top, nSize), {
      ...
      color: '#fff', align: 'center', maxWidth: w - px(160)
    });
    ...
    H.text(f.time + '   ' + f.ascentUpper + '   ' + f.third.value.toUpperCase(),
      cx, H.bl(dTop, dSize), {
        ...
        color: 'rgba(255,255,255,.7)', align: 'center', maxWidth: w - px(160)
      });
  }
});
